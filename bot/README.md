...
- **`/admin`** — только для админов: список последних заявок и ссылка на веб-админку (`/admin-simple`). Подробно: [НАСТРОЙКА_АДМИНА.md](./НАСТРОЙКА_АДМИНА.md). Админку также можно открыть из Mini App (кнопка «👑 Админка» на главной у админов) и с главной страницы бота в браузере.
