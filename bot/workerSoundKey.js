...
Язык песни и расшифровки: ${langLabel}`;
    ...
    if (!send.ok) {
      ...
      // Предложение открыть текстовую расшифровку запроса (первый раз бесплатно) — с кнопкой
      const transcriptHint =
        "📜 Хочешь получить текстовую расшифровку запроса к этой песне? Первый раз — бесплатно.";
      try {
        await fetch(`https://api.telegram.org/bot${BOT_TOKEN}/sendMessage`, {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({
            ...
            text: transcriptHint,
            reply_markup: {
              inline_keyboard: [[{ text: "📜 Получить расшифровку", callback_data: "get_analysis" }]],
            },
          })
        });
      } catch (e) {
