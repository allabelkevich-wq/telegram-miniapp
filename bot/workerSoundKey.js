...
Язык песни и расшифровки: ${langLabel}`;
    ...
    if (!send.ok) {
      ...
      // Предложение открыть текстовую расшифровку запроса (первый раз бесплатно)
      const transcriptHint =
        "📜 Хочешь получить текстовую расшифровку запроса к этой песне? Напиши в чат «расшифровка» — первый раз бесплатно.";
      try {
        await fetch(`https://api.telegram.org/bot${BOT_TOKEN}/sendMessage`, {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({ chat_id: request.telegram_user_id, text: transcriptHint })
        });
      } catch (e) {
