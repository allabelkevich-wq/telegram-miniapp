...
Всё на **Render** (`telegram-miniapp-ar09.onrender.com`): бот, API, Mini App, админка. Vercel и папка `backend/` не используются.

1. **Пуш на GitHub** — изменения деплоятся на Render автоматически.
2. **Проверка** — открыть Mini App из бота (кнопка меню), отправить заявку → в чате с ботом должна прийти песня.

Дальше — Этап 2 (пайплайн генерации) или этапы 3–5 (сайт, админка, рефералка).
...
- [x] Развернуть бэкенд на Render (бот + API в одном сервисе), `HEROES_API_BASE` задан в index.html.
