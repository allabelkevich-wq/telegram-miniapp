...
- [x] **Астрологический движок**
  - [x] Выбрать и подключить библиотеку для сидерической натальной карты (Placidus + Lahiri).
  - [x] Обернуть её в отдельный модуль astroLib (Node/TS).
  - [x] Реализовать функцию: данные рождения → подробный AstroSnapshot.
  - [x] Сохранять результаты в astro_snapshots и привязывать к track_requests.

- [x] **Система управления промптами**
  - [x] Создать таблицу prompt_templates в Supabase.
  - [x] Реализовать модуль загрузки промптов из БД (по name и is_active).
  - [x] Реализовать функцию подстановки переменных в шаблоны промптов.
  - [x] Создать начальные версии промптов:
    - [x] deepseek_archetype_v1 (архетип на основе натальной карты),
    - [x] deepseek_lyrics_v1 (текст песни),
    - [x] deepseek_title_v1 (название трека),
    - [x] suno_config_v1 (параметры для SUNO).
...
## Этап 3 — Веб‑сайт (Next.js, хостинг на выбор)
...
- [x] **Тариф «Мастер» — модуль «Мои герои»** (см. [SPEC_MOI_GEROI_MASTER.md](./SPEC_MOI_GEROI_MASTER.md))
  - [x] Миграция: app_users, clients, track_requests.client_id (файл + скрипт `npm run run-migration`; выполнить вручную в Supabase или через DATABASE_URL).
  - [x] HTTP API в боте: /api/me, /api/heroes (CRUD), валидация initData.
  - [x] Раздел «Мои герои» в Mini App при tariff === 'master'; кнопка на главной.
  - [x] CRUD героев: список, поиск, добавление, редактирование, удаление с подтверждением.
  - [x] В форме заявки: селект «Для кого создаём?»; при выборе героя подставляются данные; в заявку пишется client_id.
  - [ ] (Позже) История генераций в карточке героя.
