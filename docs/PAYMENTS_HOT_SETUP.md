...
- HOT должен отправлять подпись в `x-hot-signature` (или `x-signature`). Если `HOT_WEBHOOK_SECRET` задан, запросы без заголовка подписи отклоняются (401).
- Бэкенд проверяет HMAC SHA-256 через `HOT_WEBHOOK_SECRET`.
