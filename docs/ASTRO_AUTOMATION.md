...
### 1.2 Системы (для анализа «МОЯ ДУША»)

- **Зодиак:** сидерический (Lahiri ayanamsa). Тропическая карта не подходит — знаки смещаются.
- **Дома:** **Whole Sign** (целознаковые). Плацидус/Кох не используются для этого алгоритма.
- **Эфемериды:** Swiss Ephemeris (Moshier для расчёта без скачивания ephe-файлов).
- **Обязательно в выводе:** накшатры и пады, аспекты (в т.ч. квинтиль 72°, биквинтиль 144°, минорные), Лилит (Чёрная Луна), Хирон, Вертекс, Парс Фортуны, чара-караки (Атмакарака и др.), арудха-лагна.
