...
Проверка DeepSeek и Suno (из корня проекта):
```bash
cd bot && node test-apis.js
```
Важно: сначала `cd bot`, скрипт лежит в папке `bot/`.
...
В **index.html** задай `window.HEROES_API_BASE = 'https://telegram-miniapp-ar09.onrender.com'` (Render). Либо используй только sendData — тогда заявка уходит боту напрямую через Telegram.
