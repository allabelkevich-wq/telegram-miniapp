...
> ⚠️ **Не используется.** API встроен в `bot/index.js` и деплоится на **Render** вместе с ботом. Папка `backend/` — остаток старой схемы (отдельный деплой API на Vercel). Для продакшена см. `docs/ДЕПЛОЙ_НА_RENDER.md`.

Бэкенд для Mini App: приём заявки, валидация initData, мок-генерация «песни» и **отправка результата пользователю в Telegram**.

## Быстрый старт (только для локальной разработки)
...
## Деплой на Vercel (устарело)

> В продакшене используется Render. Инструкция ниже — только если нужен отдельный деплой API.

**Подробная пошаговая инструкция**: см. **[DEPLOY_VERCEL.md](./DEPLOY_VERCEL.md)**.
